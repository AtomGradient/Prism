#!/usr/bin/env python3
"""
Prism v2 — 消融实验 & 模型规模实验
统一 llama.cpp /v1/chat/completions API。
只负责收集模型原始输出，不评分（评分由 Claude Code 在 Step 5 中完成）。
...
def main():
    parser = argparse.ArgumentParser(description="Prism v2 消融/规模实验")
    parser.add_argument("--experiment", required=True, choices=["ablation", "scale"],
                        help="实验类型: ablation(跨域消融) 或 scale(模型规模)")
    parser.add_argument("--user", help="指定用户ID (如 user_01)")
    parser.add_argument("--all_users", action="store_true", help="运行全部10个用户")
    parser.add_argument("--data_dir", default="data/users", help="数据目录")
    ...
    parser.add_argument("--output_dir", help="输出目录 (默认 results/{experiment}/raw)")
    args = parser.parse_args()

    if not args.output_dir:
        args.output_dir = f"results/{args.experiment}/raw"
