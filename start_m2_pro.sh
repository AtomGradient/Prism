...
# ============================================================
# Prism v2 — 机器3 启动脚本
# 设备: M2 Pro 32GB | IP: 192.168.0.104
# 推理引擎: llama.cpp (统一)
# 量化: Q8 GGUF
#
# 用法: bash start_m2_pro.sh [模型选项]
#
# 模型选项:
#   9b    → Qwen3.5-9B Q8 (中端推理, 端口9200)
#   stop  → 停止所有服务
#
# 注意: M2 Pro 32GB 仅支持 9B 模型
# ============================================================

set -e

# ── 路径配置 ──────────────────────────────────────────────────
MLX_ROOT=/Users/atomgradient/Documents/mlx-community
...
# ── 启动数据节点 ──────────────────────────────────────────────
start_data_node() {
    if lsof -ti:9212 > /dev/null 2>&1; then
        warn "数据节点(9212)已运行"
        return 0
    fi

    log "启动数据节点: 模拟 iPad Air M3 (端口 9212)"
    log "  数据来源: Narrus (阅读) + Dailyn (记账)"
    ...
    nohup $PYTHON $SIM_DIR/04_lan_protocol.py \
        --role data_node \
        --node_name "M2Pro-iPad-Narrus-Dailyn" \
        --apps narrus dailyn \
        --port 9212 \
        --data_dir $SIM_DIR/data/users \
        --user_id user_01 \
        > $LOG_DIR/data_node_9212.log 2>&1 &

    local PID=$!
    echo $PID > $LOG_DIR/data_node_9212.pid
    sleep 3

    if curl -s http://localhost:9212/health > /dev/null 2>&1; then
        log "✅ 数据节点就绪 PID: $PID"
    else
        warn "数据节点启动中"
    fi
...
case "$MODEL_CHOICE" in
    ...
    stop)
        stop_all_models
        if lsof -ti:9212 > /dev/null 2>&1; then
            kill $(lsof -ti:9212) 2>/dev/null || true
            log "已停止数据节点"
        fi
        exit 0
        ;;
    *)
        echo "用法: bash start_m2_pro.sh [9b|stop]"
        echo ""
...
check_service "http://localhost:9212/health"            "数据节点(iPad) → 端口 9212"

echo ""
echo "  引擎: llama.cpp | 量化: Q8 GGUF | 线程: $THREADS"
echo "  当前模型: $MODEL_CHOICE"
echo "  对外提供: Narrus(阅读) + Dailyn(记账) (http://192.168.0.104:9212)"
echo "  日志: $LOG_DIR"
echo "============================================================"
